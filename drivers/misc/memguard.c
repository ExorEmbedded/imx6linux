...
#define DEBUG_RECLAIM2(x)
#define DEBUG_USER(x)
#define DEBUG_PROFILE(x) 

//#define USE_READ_EVENT
//#define USE_THROTTLE_THREAD
//#define THROTTLE_WATCHDOG
#define RW_LOCK(x)
#define SPIN_LOCK(x)
...
#define DELTA_WITH_OVERFLOW(a,b)  (((a) < (b))? ((0xFFFFFFFFFFFFFFFF - (b)) + (a)) :  ((a) - (b)))
...
static int g_nice_filter = 0;   /* nice filter (0=no filter) */
...
RW_LOCK(rwlock_t coreinfo_lock = __RW_LOCK_UNLOCKED(coreinfo_lock));
SPIN_LOCK(static DEFINE_SPINLOCK(coreinfo_lock));
...
{
	u64 read_val = perf_event_count(cinfo->read_event);
	return DELTA_WITH_OVERFLOW(read_val, cinfo->old_read_val);
}
#endif
...
{
	u64 read_val = perf_event_count(cinfo->write_event);
	return DELTA_WITH_OVERFLOW(read_val, cinfo->old_write_val);
}
...
void update_statistics(struct core_info *cinfo)
{
	...
	u64 read_new, write_new;
	u64 read_used, write_used;
	...
	read_new = 0;
#endif
	read_used = DELTA_WITH_OVERFLOW(read_new, cinfo->old_read_val); 
	...
	write_used = DELTA_WITH_OVERFLOW(write_new, cinfo->old_write_val); 
	...
	if (cinfo->prev_read_throttle_error && read_used < cinfo->read_budget) {
		...
		trace_printk("ERR: throttled_error: %llu < %d\n", read_used, cinfo->read_budget);
...
static void memguard_write_process_overflow(struct irq_work *entry)
{
	...
	struct memguard_info *global = &memguard_info;
	ktime_t start;
	u64 write_budget_used;
	int cur_write_budget;
	int write_budget;
	RW_LOCK(unsigned long flags);
	SPIN_LOCK(unsigned long flags);
	...
	RW_LOCK(read_lock_irqsave(&coreinfo_lock, flags));
	cur_write_budget = cinfo->cur_write_budget;
	write_budget = cinfo->write_budget;
	RW_LOCK(read_unlock_irqrestore(&coreinfo_lock, flags));

	if (write_budget_used < cur_write_budget)
	{
	    SPIN_LOCK(spin_unlock_irqrestore(&coreinfo_lock, flags));
		pr_info(">>> ERR: overflow in write timer. used %lld < cur_budget %d. ignore\n",
			     write_budget_used, cur_write_budget);
		return;
	}
	...
	if (g_use_reclaim && (!cinfo->rtcore)) {
		...
		/* reclaim back the budget I donated (if exist) */
		if (cur_write_budget < write_budget) {
			amount = write_budget - cur_write_budget;

			RW_LOCK(write_lock_irqsave(&coreinfo_lock, flags));
			cinfo->cur_write_budget += amount;
			cur_write_budget = cinfo->cur_write_budget;
			RW_LOCK(write_unlock_irqrestore(&coreinfo_lock, flags));

			local64_set(&cinfo->write_event->hw.period_left, amount);
		    SPIN_LOCK(spin_unlock_irqrestore(&coreinfo_lock, flags));
	
			DEBUG_RECLAIM(trace_printk("core %d: locally reclaimed %d (new wr: %d)\n",
						   smp_processor_id(), amount, cur_write_budget));
			return;
		}
		/* try to reclaim from the global pool */
		amount = 0;
		for_each_online_cpu(i) {
			...
			RW_LOCK(read_lock_irqsave(&coreinfo_lock, flags));
			cur_write_budget = ci->cur_write_budget;
			write_budget = ci->write_budget;
			RW_LOCK(read_unlock_irqrestore(&coreinfo_lock, flags));

			if (cur_write_budget < write_budget) {
				/* reclaim other core's donated budget */
				int tmp = write_budget - cur_write_budget;
				...
				DEBUG_RECLAIM(trace_printk("core %d: globally reclaimed new wr: %d\n",
						   i, cur_write_budget));
		...
		if (amount > 0) {
			...
			DEBUG_RECLAIM(trace_printk("core %d: globally reclaimed %d\n",
						   smp_processor_id(), amount));
			return;
		}
	}
	...
	if (write_budget_used < write_budget) {
		pr_info(">>> core %d: ERR: throttling error\n", smp_processor_id());
	...
	DEBUG_RECLAIM(trace_printk("core %d: fail to reclaim after %lld nsec. throttle %s\n",
				   smp_processor_id(), TM_NS(ktime_get()) - TM_NS(start), current->comm));
...
enum hrtimer_restart period_timer_callback_master(struct hrtimer *timer)
{
	...
	if (orun > 1)
	{
		DEBUG(pr_info(">>> ERR: timer overrun %d at period %ld\n", orun, (long)cinfo->period_cnt));
	}
...
static void period_timer_callback_slave(struct core_info *cinfo)
{
	...
	/* update statistics. */
	// AMGALBU update_statistics(cinfo);
	...
	RW_LOCK(read_lock_irqsave(&coreinfo_lock, flags));
#ifdef USE_READ_EVENT
	read_budget = cinfo->read_budget;
#endif	
	write_budget = cinfo->write_budget;
	RW_LOCK(read_unlock_irqrestore(&coreinfo_lock, flags));

#ifdef USE_READ_EVENT
	if (cinfo->read_event->hw.sample_period != read_budget) {
		/* new budget is assigned */
		trace_printk("MSG: new budget %d is assigned\n",  read_budget);
		cinfo->read_event->hw.sample_period = read_budget;
	}
#endif	
	if (cinfo->write_event->hw.sample_period != write_budget) {
		/* new budget is assigned */
		trace_printk("MSG: new write budget %d is assigned\n", write_budget);
		cinfo->write_event->hw.sample_period = write_budget;
	}
    ...
    if (g_use_reclaim && (!cinfo->rtcore)) {
...
#ifdef USE_READ_EVENT		
		cur_read_budget = cinfo->cur_read_budget;
		if (sur_rd != 0)
		{
			DEBUG_RECLAIM2(trace_printk("core %d: donated %d (new rd: %d)\n", smp_processor_id(), sur_rd, cinfo->cur_read_budget));
		}
#endif
		cur_write_budget = cinfo->cur_write_budget;
		RW_LOCK(write_unlock_irqrestore(&coreinfo_lock, flags));
		...
		if (sur_wr != 0)
		{
			DEBUG_RECLAIM2(trace_printk("core %d: donated %d (new wr: %d)\n", smp_processor_id(), sur_wr, cinfo->cur_write_budget));
		}
    } else 
    {
		RW_LOCK(write_lock_irqsave(&coreinfo_lock, flags));
	...
	{
		DEBUG_RECLAIM(trace_printk("core %d: release throttle\n", smp_processor_id()));
	}
	...
	if (!cinfo->rtcore) {
		/* setup an interrupt */
#ifdef USE_READ_EVENT		
		local64_set(&cinfo->read_event->hw.period_left, cur_read_budget);
#endif
		local64_set(&cinfo->write_event->hw.period_left, cur_write_budget);
	}
 ...
 * Local Functions
 **************************************************************************/
static int memguard_init_module(void);
static void memguard_cleanup_module(void);
				    ...
				    size_t cnt, loff_t *ppos)
{
	...
	if (!strncmp(p, "rt ", 3)) {
		...
		pr_info("Setting core %d to RT %d\n", i, val);

		if (i >=0 && i < num_online_cpus())
			per_cpu_ptr(core_info, i)->rtcore = val;
	}
	else if (!strncmp(p, "period ", 7))
	{
		sscanf(p+7, "%d", &g_period_us);
		pr_info("Setting period %d\n", g_period_us);
	}
	else if (!strncmp(p, "reclaim ", 8))
	{
		sscanf(p+8, "%d", &g_use_reclaim);
		pr_info("Setting reclaim %d\n", g_use_reclaim);
	}
	else if (!strncmp(p, "nicefilter ", 11))
	{
		sscanf(p+11, "%d", &g_nice_filter);
		pr_info("Setting nice filter %d\n", g_nice_filter);
	}
	else if (!strncmp(p, "exclusive ", 10))
	{
		sscanf(p+10, "%d", &g_use_exclusive);
		pr_info("Setting exclusive %d\n", g_use_reclaim);
	}
	else if (!strncmp(p, "enable ", 7))
	{
		sscanf(p+7, "%d", &g_enabled);
		pr_info("Setting enable %d\n", g_enabled);
		if (g_enabled && (!g_initialized))
		{
			pr_info("Initializing module\n");
			g_initialized = (memguard_init_module() == 0);
		}
	}
	else if (!strncmp(p, "init ", 5))
	{
		sscanf(p+5, "%d", &initialize);
		pr_info("Setting initialize %d\n", initialize);
		if (initialize)
		{
			if (!g_initialized)
			{
...
int memguard_core_is_throttled(void)
{
#ifndef USE_THROTTLE_THREAD	
	...
	//pr_info("throttled_task %d\n", (cinfo->throttled_task != NULL)? cinfo->throttled_task->pid : -1);

	if (cinfo == NULL)
		return 0;

#ifdef THROTTLE_WATCHDOG
	if (cinfo->throttled_task != NULL)
	{
		int64_t delta_ns = (TM_NS(ktime_get()) - TM_NS(cinfo->throttled_time));
		if (delta_ns > (2*g_period_us*1000L))
		{
			pr_info(">>> Throttle time exceeded core [%d]!!! \n", smp_processor_id());
			DEBUG_RECLAIM(trace_printk("core %d: release throttle\n", smp_processor_id()));
			cinfo->throttled_task = NULL;
		}
	}
	...
	/*
	if ((cinfo == NULL) || (cinfo->throttled_task == NULL))
		return 0;
	*/
...
int memguard_process_is_throttled(struct task_struct* p, int core_throttled)
{
	int niceness;
	struct core_info *cinfo;
	...
	// task possibly throttled. check if nice filter has to be applied
	if (g_nice_filter != 0)
	{
		if (p == NULL)
		{
			// no process (paranoid check), core is throttled
			//pr_info("throttled [2] PID [%d], nice: [%d]\n", smp_processor_id(), g_nice_filter);
			return 0;
		}	

		niceness = (p->prio - 120);
		if (niceness != g_nice_filter)
		{
			// nice filter matches: throttle core
			//pr_info("throttled [3] PID [%d], pid: [%d], nice: [%d]/[%d]\n", smp_processor_id(), p->pid, niceness, g_nice_filter);
			return 0;
		}
	}
	...
	return ((cinfo != NULL) && (cinfo->throttled_task != NULL));
	*/

	cinfo = this_cpu_ptr(core_info);
	trace_printk("Process throttled cur_write_budget: %d, write_budget: %d, write_used: %d\n", cinfo->cur_write_budget, cinfo->write_budget, cinfo->write_used[PREDICTOR]);
	return 1;
}
...
static int memguard_init_module()
{
	int i;
	...
	for_each_online_cpu(i) {
		...
		pr_info("Read budget: %d, write budget: %d\n", read_budget, write_budget);
...
#else
		cinfo->read_budget = cinfo->read_limit = 0;		
#endif		
    	...
    	atomic_set(&cinfo->write_pending.flags, atomic_read(&cinfo->write_pending.flags) | IRQ_WORK_HARD_IRQ);
...
static __init int memguard_init( void )
{
	pr_info("memguard_init\n");
	memguard_init_debugfs();
	return 0;
}

static __exit void memguard_cleanup( void )
{
	if (g_initialized)
		memguard_cleanup_module();
...
module_init(memguard_init);
module_exit(memguard_cleanup);
MODULE_LICENSE("GPL");
